/* =========================
   IMPORTS
========================= */
import React from "react";
import { useNavigate } from "react-router-dom";
import {
  IoSearchOutline,
  IoFileTrayFullOutline,
  IoAlertCircle,
  IoAlbumsOutline,
} from "react-icons/io5";
import usePageMetadata from "../../hooks/usePageMetadata";
import Header from "../../components/NavigationBars/Header";
import Footer from "../../components/NavigationBars/Footer";

/* =========================
   COMPONENT
========================= */
function Dashboard() {
  usePageMetadata("Dashboard", "/images/LAFLogo.png");

  const navigate = useNavigate();

  /* =========================
     HANDLERS
  ========================= */
  const goToReportLost = () => navigate("/ReportLostItemPage");
  const goToFoundItems = () => navigate("/FoundItemPage");
  const goToLostItems = () => navigate("/LostItemPage");

  /* =========================
     RENDER
  ========================= */
  return (
    <div style={styles.container}>
      <Header />

      <div style={styles.dashboardContainer}>
        {/* HEADER TEXT */}
        <div style={styles.text}>
          <p style={styles.systemName}>USTP LOST AND FOUND</p>
          <p>The community platform that connects people who lost</p>
          <p>with those who have found them</p>
        </div>

        {/* ACTION BUTTONS */}
        <div style={styles.buttonContainer}>
          <button style={styles.buttonBlue} onClick={goToReportLost}>
            <IoAlertCircle size={40} color="#EF4444" />
            Report an Item
          </button>

          <button style={styles.button} onClick={goToFoundItems}>
            <IoFileTrayFullOutline size={40} color="#64748B" />
            Search Found Items
          </button>
        </div>

        {/* CARDS */}
        <div style={styles.cardContainer}>
          <div style={styles.card} onClick={goToReportLost}>
            <IoAlertCircle size={50} color="#EF4444" />
            <h5 style={styles.cardTitle}>Report Lost Items</h5>
            <p style={styles.cardText}>Report your lost item here.</p>
          </div>

          <div style={styles.card} onClick={goToFoundItems}>
            <IoSearchOutline size={50} color="black" />
            <h5 style={styles.cardTitle}>Found Item</h5>
            <p style={styles.cardText}>Find found items here.</p>
          </div>

          <div style={styles.card} onClick={goToLostItems}>
            <IoAlbumsOutline size={50} color="black" />
            <h5 style={styles.cardTitle}>Lost Item</h5>
            <p style={styles.cardText}>Find your lost items here.</p>
          </div>
        </div>
      </div>

      <Footer />
    </div>
  );
}

/* =========================
   STYLES
========================= */
const styles = {
  container: {
    display: "flex",
    flexDirection: "column",
    minHeight: "100vh",
    fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif",
    background: "linear-gradient(135deg, #f6f8ff 0%, #f0f2ff 100%)",
  },

  dashboardContainer: {
    display: "flex",
    flexGrow: 1,
    flexDirection: "column",
    alignItems: "center",
    padding: "20px 40px",
    margin: "0px 20px",
    minHeight: "65vh",
  },

  text: {
    textAlign: "center",
    color: "#64748B",
    marginBottom: "40px",
  },

  systemName: {
    fontSize: 50,
    fontWeight: "bold",
    color: "#0F172A",
  },

  buttonContainer: {
    display: "flex",
    justifyContent: "center",
    gap: "10px",
    margin: "10px 0px",
  },

  buttonBlue: {
    display: "flex",
    alignItems: "center",
    gap: "10px",
    padding: "20px",
    backgroundColor: "#1A1851",
    color: "#fff",
    border: "1px solid #1A1851",
    borderRadius: "15px",
    cursor: "pointer",
    fontSize: "16px",
  },

  button: {
    display: "flex",
    alignItems: "center",
    gap: "10px",
    padding: "20px",
    backgroundColor: "#FFFFFF",
    color: "#64748B",
    border: "1px solid #FCB315",
    borderRadius: "15px",
    cursor: "pointer",
    fontSize: "16px",
  },

  cardContainer: {
    display: "flex",
    justifyContent: "space-between",
    gap: "30px",
    margin: "10px 40px",
    padding: "0px 20px",
  },

  card: {
    backgroundColor: "#F3EDED",
    borderRadius: "15px",
    padding: "40px 20px 20px",
    width: "30%",
    textAlign: "center",
    boxShadow: "0 4px 8px rgba(0,0,0,0.1)",
    cursor: "pointer",
  },

  cardTitle: {
    fontWeight: "bold",
    fontSize: 25,
  },

  cardText: {
    fontSize: 20,
    color: "#64748B",
    margin: "0px 10px",
  },
};

export default Dashboard;
